import { Bell, Clock, FileText, Layers, ShieldCheck } from "lucide-react";
import { notificationItems } from "@/data/notifications";

const statusStyles: Record<string, string> = {
  "Amendment detected": "bg-amber-100 text-amber-700 border-amber-200",
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-ink-500">
                Track document amendments, evidence updates, and review tasks across regulatory workspaces.
              </p>
            </div>
          </div>
        </section>
              ...
              >
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div className="min-w-0 flex-1">
                    ...
                    <h2 className="text-base font-semibold text-ink-900">{item.documentName}</h2>
                    <p className="mt-2 text-sm leading-relaxed text-ink-600">{item.description}</p>
                  </div>

                  <div className="grid gap-2 text-xs text-ink-500 sm:grid-cols-3 lg:w-80 lg:grid-cols-1">
                    <div className="flex items-center gap-2">
                      <FileText className="h-3.5 w-3.5 text-ink-400" />
                      <span className="font-medium text-ink-600">Document:</span>
                      <span className="truncate">{item.documentName}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Layers className="h-3.5 w-3.5 text-ink-400" />
                      <span className="font-medium text-ink-600">Workspace:</span>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-ink-600">Time:</span>
